...
const ModalDayBook = ({
  ...
  // Handle checkbox change without automatic edit
  const handleCheckboxChangeWithEdit = (item) => {
    handleCheckboxChange(item);
    // Don't automatically trigger edit - let user click OK button
  };
  ...
  const getSummary = (items = []) => {
    if (typeof calculateSums === "function") {
      try {
        ...
        console.error("Purchase DayBook: calculateSums failed", error);
      }
    }
  ...
  const buildExportRows = () => {
    ...
    return effectiveRows.map((item, exportIndex) => {
      ...
      const position = Array.isArray(data)
        ? data.findIndex((purchaseItem) => purchaseItem._id === item._id)
        : -1;
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Purchase DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `purchase_daybook_${timestamp}.xlsx`);
  };
                    ...
                    >
                      ...
                      >
                        <Checkbox
                          ...
                          onChange={() => handleCheckboxChangeWithEdit(item)}
                        />
                        <Typography
                          sx={{
                            color: "var(--Main-Text, #343434)",
                      ...
                      >
                        ...
                        >
                          {/* Pcs */}
                          {item?.items ? calculateSums(item.items).pcs : (item.pcs || 0)}
                        </Typography>
                      </Box>
                  ...
                  if (isOkButtonEnabled) {
                    ...
                    if (selectedItem) {
                      ...
                      const editData = {
                        ...
                        isFromMemoPending: true // Mark as coming from DayBook
                      };
