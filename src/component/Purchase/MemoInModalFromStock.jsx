...
import { memoInfoState } from "recoil/Purchase/MemoState";
...
const MemoInModalFromStock = ({ data, state, handleSubmit, disabled = false , isApproved = false }) => {
  ...
  const handleCheckboxSelectAllChange = () => {
    if (selectedRows.length === rowData.length) {
      ...
      // Select all - filter out any rows that are already selected to prevent duplicates
      setSelectedRows((prev) => {
        const existingIds = new Set(prev.map(row => row._id));
        const newRows = rowData.filter(row => !existingIds.has(row._id));
        return [...prev, ...newRows];
      });
    }
  };
                ...
                {loading ? (
                  ...
                  rowData.map((row, rowIndex) => (
                    ...
                    >
                      ...
                      >
                        <Typography sx={textStyle}>{row.memo_no}</Typography>
                      </Box>
                      <Box
                        sx={{
                          width: "140px",
...
export default MemoInModalFromStock;
