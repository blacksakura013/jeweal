...
const QuotationsOrderHeader = ({
  ...
  const handleApprovalAction = async () => {
    ...
    try {
      // For PO data, we need to save first, then approve
      if (memoInfo?.isPOEdit) {
        ...
        manuallyApproved.current = true;
        
        // First save the PU (this will create a new PU and return the ID)
        if (handleSaveMemo) {
          const savedPU = await handleSaveMemo(true); // Skip reload for approval process
          
          // After saving, approve the newly created PU
          if (savedPU && savedPU.id) {
            try {
            const response = await apiRequest("PUT", `/pu/${savedPU.id}`, {
              status: "approved",
              invoice_no: memoInfo?.invoice_no || ""
              });
              
              if (response) {
                // Update memoInfo with the new ID
                setMemoInfo(prev => ({
                    ...prev,
                    id: savedPU.id
                }));
                ...
                manuallyApproved.current = false;
              }
            } catch (error) {
            ...
            // If save fails, revert the approval state
            setIsApproved(false);
            setMemoInfo(prev => ({
              ...prev,
              status: "pending"
          ...
          // If handleSaveMemo is not available, revert the approval state
          setIsApproved(false);
          setMemoInfo(prev => ({
            ...prev,
            status: "pending"
        ...
        // For DayBook data, use existing PU ID
        if (!currentPUId) {
        ...
        <Box sx={{ display: "flex", alignItems: "center" }}>
          <Box sx={{ marginRight: "12px" }}>
            <Button
              ...
              onClick={() => {
                if (!isApproved && (currentPUId || memoInfo?.isPOEdit) && !editMemoStatus) {
              ...
              sx={{
                ...
                borderRadius: "4px",
                gap: "8px",
                backgroundColor: (!currentPUId || editMemoStatus)
                  ? "#E6E6E6" 
                  : isApproved 
                    ? "#00AA3A33" 
                    : "#C6A96933",
                border: (!currentPUId || editMemoStatus)
                  ? "1px solid #BFBFBF" 
                  : isApproved 
                    ? "1px solid #00AA3A80" 
                    : "1px solid #C6A96980",
                color: (!currentPUId || editMemoStatus)
                  ? "#57646E" 
                  : isApproved 
                    ? "#00AA3A"
                    : "#C6A969",
