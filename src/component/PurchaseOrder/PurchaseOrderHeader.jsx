...
import { DayBookQuotationState } from "recoil/PurchaseOrder/PurchaseOrderState";
...
const PurchaseOrderHeader = ({
  ...
  fsmState, // Add FSM state
  hasUnsavedData, // Add function to check unsaved data
}) => {
  ...
  useEffect(() => {
    if (memoInfo?.id && (memoInfo?.isDayBookEdit || memoInfo?.status)) {
  ...
  // Reset approval state when component unmounts or when starting new transaction
  useEffect(() => {
    return () => {
      // Reset approval state when component unmounts
      setIsApproved(false);
    };
  }, []);
  ...
  const getInventoryData = async () => {
    try {
      const response = await apiRequest("GET", "/po");
  ...
  const handleDayBookSubmit = () => {
    ...
    // Filter selected items directly from the main array
    const selectedItems = inventoryData.filter((item) => item.checked);

    if (selectedItems.length > 0) {
      // Take the first selected item
      const selectedItem = selectedItems[0];
      inventoryDataSelected.push(selectedItem);
    }
    ...
    const remainSelected = inventoryDataSelected.filter(
      (el) => !currentSelectedItems.map((el) => el._id).includes(el._id)
    );
    currentSelectedItems = currentSelectedItems.concat(remainSelected);
    ...
    if (
      ...
      setPendingDayBookSelection(currentSelectedItems);
      setIsOpenUnsavedDataDialog(true);
      return;
    }

    setDayBookQuotation(currentSelectedItems[0]);
    setIsDisableDayBook(true);
    handleSaveMemo(currentSelectedItems);
  };
  ...
  // Function to handle approval API call (no unapprove functionality)
  const handleApprovalAction = async () => {
    if (!currentPOId || isApproved) {
      console.error(
        "No PO ID available for approval action or already approved"
      );
      return;
    }
    ...
    setIsApproved(true);
    // Update memoInfo with new status immediately
    setMemoInfo((prev) => ({
      ...prev,
      status: "approved",
    }));
    // Disable edit mode after approval
    setEditMemoStatus(false);

    try {
      const response = await apiRequest("PUT", `/po/${currentPOId}/approve`, {
        status: "approved",
    ...
    <>
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center" }}>
          <Box sx={{ marginRight: "12px" }}>
            <Button
              disabled={!currentPOId || isApproved || editMemoStatus}
              onClick={() => {
                if (currentPOId && !isApproved && !editMemoStatus) {
      ...
      >
        ...
        >
          ...
          >
            You have unsaved changes. Do you want to discard them and load
            daybook data?
          </Typography>
          <Box sx={{ display: "flex", gap: "14px" }}>
            <Button
