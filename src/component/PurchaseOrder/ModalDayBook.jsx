...
const ModalDayBook = ({
  ...
  const getSummary = (items = []) => {
    if (typeof calculateSums === "function") {
      try {
        ...
        console.error("Purchase Order DayBook: calculateSums failed", error);
      }
    }
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "PO DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `purchase_order_daybook_${timestamp}.xlsx`);
  };
