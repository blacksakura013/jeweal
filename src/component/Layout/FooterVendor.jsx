...
const handleConfirmClose = async (confirmed) => {
  ...
  if ((props.type === "quotation" || props.type === "reserve" || props.type === "purchaseOrder" || props.type === "purchase" || props.type === "memo_out"  || props.type === "memo_out_return" || props.type === "load") && props.onSaveClick) {
    setOpenConfirm(false);
    props.onSaveClick(); 
    return;
  }
  ...
  try {
    ...
    const recordId = props.formData?._id || props.selectedData?._id;
    ...
    const processData = await apiRequest(method, endpoint, {
      ...updatedVendorData,
  ...
  }, [openSuccess, props.onSaveSuccess]);
  ...
  const getCancelDisabled = () => {
    ...
    // For load type: if approved, disable cancel
  ...
  const getSaveDisabled = () => {
    ...
    // For load type: if approved, disable save
    if (props.type === "load" && isApproved) {
      return true;
    }
    ...
    if (isCancelled) {
      return true; // Disable save when cancelled
    }
    ...
    if (props.fsmState) {
      ...
      if (props.type === "purchase" || props.type === "purchaseOrder" || props.type === "memo-return" || props.type === "load") {
