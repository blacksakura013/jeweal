  ...
  const handleStockSubmit = (selectedStockRows) => {
    ...
    const newRows = selectedStockRows.map((stockRow, index) => {
      ...
      return {
        ...
        labour: "",
        labour_price: 0,
        isFromStock: false, // Mark as NOT from reserve (this is from stock)
    ...
    setRows((prevRows) => {
      // Only check for duplicates if rows are from stock
      // Note: Sale uses isFromStock: false for stock items, so we check by id/stock_id presence
      // Create a Set of existing stock IDs from rows that came from stock
      const existingStockIds = new Set(
        prevRows
          .filter(row => (row.id || row.stock_id)) // Check rows that have stock identifiers
  ...
  return (
    <>
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        ...
        <QuotationModalFromStock handleSubmit={handleStockSubmit} disabled={disabled} />
