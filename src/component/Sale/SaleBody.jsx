...
const SaleBody = ({
  ...
  const isCancelledDoc = ((memoInfo?.status || "") + "").toLowerCase() === "cancelled" || ((memoInfo?.status_cancel || "") + "").toLowerCase() === "cancelled";
  const shouldDisableFromStock = memoInfo?.isDayBookEdit ? false : rows?.some(item => item?.isFromStock);
  const shouldDisableFields = (fsmState === "saved" && !editMemoStatus) || (memoInfo?.isDayBookEdit && !editMemoStatus);
  const isDisabled = isCancelledDoc || (memoInfo?.isDayBookEdit && !editMemoStatus) || (shouldDisableFromStock && !editMemoStatus) || shouldDisableFields  ;
