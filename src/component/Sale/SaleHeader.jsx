...
import { editMemoState, memoInfoState } from "recoil/Sale/MemoState.js";
import { DayBookQuotationState } from "recoil/Sale/SaleState";
...
const SalerHeader = ({
  ...
  const getInventoryData = async () => {
    try {
      const response = await apiRequest("GET", "/sales");

      const inventoryData = response.map((el) => {
        return {
          ...el,
          vendor_code_id: el.vendor_code_id, 
  ...
  useEffect(() => {
    if (memoInfo?.id) {
      setCurrentSaleId(memoInfo.id);
    } else {
      setCurrentSaleId(null);
    }

    const statusStr = ((memoInfo?.status || "") + "").toLowerCase();
    const statusCancelStr = ((memoInfo?.status_cancel || "") + "").toLowerCase();
    setIsCancelled(statusStr === "cancelled" || statusCancelStr === "cancelled");
  }, [memoInfo]);
    ...
    <>
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center" }}>
          {/* Cancel button (separate modal implementation) - Left side */}
          <Box sx={{ marginRight: "12px" }}>
            <Button
              disabled={!currentSaleId || isCancelled || editMemoStatus}
              onClick={() => {
                if (currentSaleId && !isCancelled && !editMemoStatus) setIsOpenCancelModal(true);
              }}
              sx={{
                textTransform: "none",
                height: "35px",
      ...
      {/* Cancel Confirmation Modal (inline, separate) */}
      <Dialog
          ...
          <Box sx={{ display: "flex", gap: "14px" }}>
            ...
            <Button 
              onClick={async () => {
                ...
                try {
                  const resp = await apiRequest("PUT", `/sales/${currentSaleId}/cancel`);
                  if (resp) {
                    setIsCancelled(true);
                    setMemoInfo(prev => ({ ...prev, status_cancel: "cancelled" }));
