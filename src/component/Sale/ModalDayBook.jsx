...
const ModalDayBook = ({
  ...
  // Check if OK button should be enabled (exactly one item selected)
  ...
  const isAllSelected = hasData && data.every((item) => item.checked);
  ...
  const getSummary = (items = []) => {
    if (typeof calculateSums === "function") {
      try {
        ...
        console.error("Sale DayBook: calculateSums failed", error);
      }
    }
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sale DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `sale_daybook_${timestamp}.xlsx`);
  };
  ...
  const handleSelectAll = () => {
    ...
    setSelectedItems(newCheckedState ? [...data] : []);

    data.forEach((item) => {
      if (Boolean(item.checked) !== newCheckedState) {
        handleCheckboxChange(item);
      }
    });
  };
