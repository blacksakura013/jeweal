...
const AccountVendorBodyAdd = ({
  ...
  const handleChangeShippingCountry = (newValue) => {
    setSelectedShippingCountry(newValue);
  ...
  const handleChangeShippingCity = (newValue) => {
    setSelectedShippingCity(newValue);
    setErrors((prev) => ({
      ...prev,
      city: newValue ? "" : "City is required.",
    }));
  ...
  const handleChangeShippingState = (newValue) => {
    setSelectedShippingState(newValue);
    setErrors((prev) => ({
      ...prev,
      stateProvince: newValue ? "" : "State/Province is required.",
    }));
    ...
    // เมื่อเลือกรัฐใหม่ ให้ดึงข้อมูลเมือง
    if (newValue && selectedShippingCountry) {
      const citiesData = CityData.getCitiesOfState(
  ...
  console.log("vendorListAddress", vendorListAddress);
          ...
          >
            ...
            <Box sx={{ marginLeft: "24px" }}>
              <Box sx={{ display: "flex" }}>
                ...
                >
                  <div style={{ pointerEvents: editStatus ? 'auto' : 'none' }}>
                  <IOSSwitch
                    checked={
                      vendorData.account_status == "active" ? true : false
                    }
                      disabled={!editStatus}
                      onChange={editStatus ? (event) => {
                      setVendorData({
                        type: "account_infomation",
                        account_status:
              ...
              >
                <Box sx={{ display: "flex" }}>
                  ...
                  >
                    ...
                    {/* <IOSSwitch
                      checked={
                        vendorListAddress.length > 0 && vendorListAddress[0].status === "active"
                          ? true
                          : false
                      }
                        disabled={!editStatus}
                        ...
                        if (vendorListAddress.length > 0) {
                          const updatedAddresses = [...vendorListAddress];
                          updatedAddresses[0] = {
                            ...updatedAddresses[0],
                            status: event.target.checked ? "active" : "inactive",
                          };
                          setVendorListAddress(updatedAddresses);
                        }
                        } : undefined}
                    /> */}
                    </div>
                  ...
                  {vendorListAddress?.map((v, index) => (
                    <Box key={index} mb={2}>
                      ...
                      <StateProvince
                        ...
                        options={StateData.getStatesOfCountry(
                          v?.country?.code
                        ).map((state) => ({
                          code: state.isoCode,
                          label: state.name,
                      ...
                      <City
                        ...
                        options={CityData.getCitiesOfState(
                          v?.country?.code,
                          v?.state?.code
                        ).map((city) => ({
                          code: city.name,
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {/* <IOSSwitch
                      checked={
                        vendorListShippingAddress.length > 0 && vendorListShippingAddress[0].status === "active"
                          ? true
                          : false
                      }
                        disabled={!editStatus}
                        onChange={editStatus ? (event) => {
                        if (vendorListShippingAddress.length > 0) {
                          const updatedShippingAddresses = [...vendorListShippingAddress];
                          updatedShippingAddresses[0] = {
                            ...updatedShippingAddresses[0],
                            status: event.target.checked ? "active" : "inactive",
                          };
                          setVendorListShippingAddress(updatedShippingAddresses);
                        }
                        } : undefined}
                    /> */}
                    </div>
...
export default AccountVendorBodyAdd;
