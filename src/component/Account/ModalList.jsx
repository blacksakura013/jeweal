...
const exportToExcel = (selectedList) => {
  // เลือกเฉพาะฟิลด์ที่ต้องการ เช่น code, name, และ master_status
  const filteredData = selectedList.map((item) => ({
    vendor_id: item.vendor_code_id,
    business_type: item.business_type,
    contact_person: item.contact_person,
    vendor_code_name: item.vendor_code_name,
    email: item.email,
    phone_no: item.phone_no,
  }));
  const worksheet = XLSX.utils.json_to_sheet(filteredData);
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Selected Vendor");
...
const ModalList = (props) => {
  ...
  console.log(vendorList , "vendorListState")
  ...
  const handleSelectAllChange = (event) => {
    ...
    if (isChecked) {
      // Select all items
      setSelectedCheckboxes(props.dataList);
    } else {
      // Deselect all items
      setSelectedCheckboxes([]);
    }
  ...
  const setActive = (id) => async (event) => {
    const newStatus = event.target.checked ? "active" : "inactive";
    console.log(newStatus)

    const updateStatus = await apiRequest("PUT", "/account/vendor/setactive", {
      _id: id,
      status: newStatus
    })
    console.log(updateStatus)
    setVendorList(updateStatus)
  }
  return (
    <>
      <Box>
        ...
        >
          <Box sx={style}>
            ...
            >
              ...
              <Box sx={{
                marginTop: "16px", marginRight: "16px", cursor: "pointer", "&:hover svg path": {
                  fill: "#E00410",
                },
              }} onClick={handleClose}>
                <svg
                  ...
                  vendorList?.map((v, k) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <Checkbox
                          checked={selectedCheckboxes.includes(v)}
                          onChange={(event) =>
                            handleCheckboxChange(event, v)
                          }
                        />
                        <Typography
                          sx={{
                      ...
                      >
                        ...
                        >
                          {
                          (v?.invoice_address?.length > 0) && v?.invoice_address?.[0]?.country?.label}
                      ...
                      >
                        ...
                        >
                          ...
                          <IOSSwitch onChange={setActive(v._id)} checked={v.account_status == "active"} />
                        </Typography>
                      </Box>
                    </Box>
                  ))
