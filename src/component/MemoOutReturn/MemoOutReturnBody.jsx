...
const MemoOutReturnBody = ({
  ...
  const actualEditMemoStatus = editMemoStatus !== undefined ? editMemoStatus : editMemoStatusValue;
  
 
  const shouldDisableFields = (fsmState === "saved" && !actualEditMemoStatus) || (memoInfo?.isDayBookEdit && !actualEditMemoStatus);
  ...
  const genInvoiceNo = async () => {
    try {
      const data = await apiRequest("GET", "/memo-out-returns/next-invoice-no", {});
  ...
  const getCompanyInfo = async () => {
        try {
          ...
          // Also fetch the currency code
          const currencyCode = await getCurrencyCodeById(apiRequest, companyInfoData.currency);
          
          // Only set if NOT in daybook mode and currency is not already populated
          setMemoInfo((prev)=>{
            // If in daybook mode OR currency is already set, don't overwrite
            if (prev?.isDayBookEdit || prev?.currency) {
              return prev;
            }
            return {
              ...prev,
            ...
            >
              ...
              >
                <Box sx={{ display: "flex", marginTop: "18px" }}>
                    ...
                    <DatePicker
                      ...
                      onChange={(newValue) => {
                        onDocDateChange(newValue);
                            ...
                            disabled={shouldDisableFields || (isDayBookDataLoaded && !actualEditMemoStatus)}
                      format="DD/MM/YY"
                      slotProps={{
                        textField: {
                          required: true,
                  ...
                  <Account disabled={shouldDisableFields} triggerFSMDirty={triggerFSMDirty} />
                  <InvoiceAddress
                    initialValue={
...
export default MemoOutReturnBody;
