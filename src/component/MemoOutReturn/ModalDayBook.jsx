...
const ModalDayBook = ({
  ...
  const handleOpen = () => {
    setOpen(true);
    setSelectedItemIds([]); // Reset selection when opening
  };

  const handleClose = () => {
    setOpen(false);
    setSelectedItemIds([]); // Reset selection when closing
  };
  ...
  const handleOKClick = () => {
    if (selectedItemIds.length === 1) {
      // Find the selected item and call handleEdit with it
      ...
      if (selectedItem) {
        ...
        const shouldShowDialog = 
          (editMemoStatus === true) || 
          ((fsmState === "initial" || fsmState === "dirty" || fsmState === "editing") && hasUnsavedData && hasUnsavedData());
  ...
  const getSummary = (items = []) => {
    if (typeof calculateSums === "function") {
      try {
        ...
        console.error("Memo Out Return DayBook: calculateSums failed", error);
      }
    }
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Memo Out Return DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `memo_out_return_daybook_${timestamp}.xlsx`);
  };
    ...
    <>
      <Box>
       ...
       <ConfirmCancelDialog
        ...
        message="You have unsaved changes. Do you want to proceed with selecting this data? Your current changes will be lost."
        noButtonText="No"
        yesButtonText="Yes"
      />
