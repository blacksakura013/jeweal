...
const ReturnMemoPending = ({ data, state, handleSubmit, fsmState, hasUnsavedData }) => {
  ...
  const handleCheckboxChange = (row) => {
    setSelectedRows((prev) => {
      // Use __rowIndex as the unique identifier (always unique regardless of properties)
      const rowIndex = row.__rowIndex;
      
      if (rowIndex === undefined) {
        // Fallback: if rowIndex is not set, use _id or create a unique identifier
        const rowId = row._id || `fallback_${Date.now()}_${Math.random()}`;
        const isSelected = prev.some(
          (selectedRow) => {
            if (selectedRow.__rowIndex !== undefined) return false;
            return (selectedRow._id || selectedRow.uniqueId) === rowId;
          }
        );
        
        if (isSelected) {
          return prev.filter((selectedRow) => {
            if (selectedRow.__rowIndex !== undefined) return true;
            return (selectedRow._id || selectedRow.uniqueId) !== rowId;
          });
        } else {
          const alreadyExists = prev.some((selectedRow) => {
            if (selectedRow.__rowIndex !== undefined) return false;
            return (selectedRow._id || selectedRow.uniqueId) === rowId;
          });
      ...
      // Use rowIndex for comparison (most reliable)
      const isSelected = prev.some(
        (selectedRow) => selectedRow.__rowIndex === rowIndex
      );
      
      if (isSelected) {
        // Remove if already selected
        return prev.filter((selectedRow) => selectedRow.__rowIndex !== rowIndex);
      } else {
        // Prevent duplicate - only add if not already in the array
        const alreadyExists = prev.some((selectedRow) => selectedRow.__rowIndex === rowIndex);
  ...
  const handleOpen = () => {
    if (!memoInfo?.account?.label) {
      console.log(memoInfo?.account?.label , "memoInfo?.account?.label")
  ...
  return (
    <>
      <Box>
              ...
              >
                ...
                {loading ? (
                  ...
                  rowData.map((row, rowIndex) => {
                    ...
                    return (
                    <Box
                      key={uniqueKey} // Use rowIndex for truly unique key
                      sx={{
                        width: "fit-content",
                        height: "42px",
                        bgcolor: "#FFF",
        ...
        message="You have unsaved changes. Do you want to proceed with selecting memo pending data? Your current changes will be lost."
        noButtonText="No"
        yesButtonText="Yes"
      />
    </>
...
export default ReturnMemoPending;
