...
const SearchBarStoneMaster = ({ onDataSelect, isEditing, selectedData}) => {
  ...
  useEffect(() => {
    ...
    apiRequest("GET", "/master?master_type=master_stone_group")
      .then((response) => {
        // Include both active and inactive; we'll sort later to keep inactive at the bottom
        setData(response || []);
        setLoading(false);
      })
      .catch((err) => {
  ...
  const filteredData = useMemo(() => {
    ...
    // Sort so that active items appear first, inactive items at the bottom
    return [...base].sort((a, b) => {
      const aActive = a.master_status === "active" ? 1 : 0;
      const bActive = b.master_status === "active" ? 1 : 0;
      return bActive - aActive; // active (1) before inactive (0)
    });
  }, [data, searchTerm]);
          ...
          filteredData.map((item) => (
            ...
            >
              <Typography
                sx={{
                  marginLeft: "16px",
                  color: item.master_status === "inactive" ? "#BFBFBF" : "var(--black, #000)",
                  fontFamily: "Calibri",
                  fontSize: "16px",
                  fontStyle: "normal",
                  fontWeight: 400,
                  color : selectedItemId === item._id ? "#000000" : isEditing ? "#ababab" : (item.master_status === "inactive" ? "#BFBFBF" : "#000000")
                }}
              >
                {item.code}
              </Typography>
...
export default SearchBarStoneMaster;
