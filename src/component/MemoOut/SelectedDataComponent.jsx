...
const SelectedDataComponent = ({
  ...
  // Calculate if table fields should be disabled
  // disabled prop from parent = view mode (saved and not editing)
  // In add mode (initial/dirty), disabled will be false, so fields are editable
  const tableFieldsDisabled = disabled;
  ...
  const handleStockSubmit = (selectedStockRows) => {
    ...
    const newRows = selectedStockRows.map((stockRow, index) => {
      // Format image path: if relative path, prefix with API_URL for display
      const imageUrl = stockRow.image
        ? (/^https?:\/\//.test(stockRow.image) ? stockRow.image : `${API_URL}${stockRow.image}`)
        : null;
      
      return {
        _id: `stock_${stockRow._id}_${Date.now()}_${index}`, // Generate unique ID for React operations
        stone_code: stockRow.stone_code,
        account:stockRow.account,
        type: stockRow.type,
        id: stockRow._id, // Keep original ID for API calls
        ...
        // labour_price: 0,
        isFromStock:true,
        // Include image from stock data for thumbnail display
  ...
  const headers = [
    ...
    { label: "Shape", width: "142px" ,  className : "sticky-col sticky-col-4" },
  ...
  return (
    <>
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        ...
        <QuotationModalFromStock rowData={rowData} setRowData={setRowData} handleSubmit={handleStockSubmit} />
      </Box>
