...
export const selectedRowsState = atom({
  key: "quotationSelectedRowsState",
...
const QuotationModalFromStock = ({rowData, setRowData ,  data, state, handleSubmit }) => {
  ...
  useEffect(() => {
    const fetchStock = async () => {
      try {
        ...
        // Fetch both stock data and location data in parallel
        const [stockResponse, locationResponse] = await Promise.all([
          apiRequest("GET", "/stocksandconsignments"),
          apiRequest("GET", "/sublocations")
        ]);
        
        // Create location mapping for quick lookup
        const locationMap = {};
        locationResponse.forEach(location => {
          locationMap[location._id] = location.location_name;
        });
        
        // Merge stock and consignment data and add location names
        const mergedData = [...stockResponse.stocks, ...stockResponse.consignments].map(item => ({
          ...item,
          location_name: locationMap[item.location] || item.location
        }));
        setRowData(mergedData);
        
        console.log("Stock data fetched:", mergedData);
      } catch (error) {
  ...
  console.log("Stock data fetched2:", rowData);
  ...
  const handleCheckboxClick = () => {
    ...
    console.log(rowData); // Get the object on click
  };
                ...
                >
                  ...
                  >
                    ...
                    <svg
                      ...
                      fill="none"
                    >
                      <g clip-path="url(#clip0_2622_213943)">
                        <path
                  ...
                  >
                    ...
                    <svg
                      ...
                      fill="none"
                    >
                      <g clip-path="url(#clip0_2622_213958)">
                        <path
                          d="M2 1.5H10C10.1326 1.5 10.2598 1.55268 10.3536 1.64645C10.4473 1.74021 10.5 1.86739 10.5 2V2.793C10.5 2.9256 10.4473 3.05275 10.3535 3.1465L7.1465 6.3535C7.05273 6.44725 7.00003 6.5744 7 6.707V9.8595C7 9.9355 6.98267 10.0105 6.94933 10.0788C6.91599 10.1471 6.86752 10.2069 6.80761 10.2537C6.74769 10.3004 6.6779 10.3329 6.60355 10.3486C6.52919 10.3644 6.45222 10.363 6.3785 10.3445L5.3785 10.0945C5.27038 10.0674 5.1744 10.005 5.10583 9.9171C5.03725 9.82923 5 9.72096 5 9.6095V6.707C4.99997 6.5744 4.94727 6.44725 4.8535 6.3535L1.6465 3.1465C1.55273 3.05275 1.50003 2.9256 1.5 2.793V2C1.5 1.86739 1.55268 1.74021 1.64645 1.64645C1.74021 1.55268 1.86739 1.5 2 1.5Z"
                          stroke="#666666"
                          stroke-width="1.5"
                ...
                {loading ? (
                  ...
                  >
                    ...
                    <Box sx={{ width: "140px", display: "flex", alignItems: "center", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.location_name}</Typography>
                    </Box>
                    <Box sx={{ width: "140px", display: "flex", alignItems: "center", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.type}</Typography>
                    </Box>
                    <Box sx={{ width: "140px", display: "flex", alignItems: "center", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.stone_code}</Typography>
                    </Box>
                    <Box sx={{ width: "160px", display: "flex", alignItems: "center", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.stock_id}</Typography>
                    </Box>
                    <Box sx={{ width: "90px", display: "flex", alignItems: "center", justifyContent: "start" }}>
                      <Typography sx={textStyle}>{row.lot_no}</Typography>
                    </Box>
                    <Box sx={{ width: "120px", display: "flex", alignItems: "center", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.stone}</Typography>
                    </Box>
                    <Box sx={{ width: "100px", display: "flex", alignItems: "center", padding: "12px 8px" }}>
                    ...
                    <Box sx={{ width: "100px", display: "flex", alignItems: "center", padding: "12px 8px" , justifyContent: "start"}}>
                    ...
                    <Box sx={{ width: "140px", display: "flex", alignItems: "center", padding: "12px 8px"  , justifyContent: "start"  }}>
                      <Typography sx={textStyle}>{row.cer_type}</Typography>
                    </Box>
                    <Box sx={{ width: "140px", display: "flex", alignItems: "center", padding: "12px 8px"  ,  justifyContent: "start" }}>
                      <Typography sx={textStyle}>{row.cer_no}</Typography>
                    </Box>
                    <Box sx={{ width: "60px", display: "flex", alignItems: "center", justifyContent: "start" ,  padding: "12px 8px"  }}>
                      <Typography sx={textStyle}>{row.pcs}</Typography>
                    </Box>
                    <Box sx={{ width: "120px", display: "flex", alignItems: "center", justifyContent: "flex-end", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.weight}</Typography>
                    </Box>
                    <Box sx={{ width: "120px", display: "flex", alignItems: "center", justifyContent: "flex-end", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.price}</Typography>
                    </Box>
                    <Box sx={{ width: "60px", display: "flex", alignItems: "center", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.unit_price}</Typography>
                    </Box>
                    <Box sx={{ width: "120px", display: "flex", alignItems: "center", justifyContent: "flex-end", padding: "12px 8px" }}>
                      <Typography sx={textStyle}>{row.amount}</Typography>
                    </Box>
                    <Box sx={{ width: "286px", display: "flex", alignItems: "center", padding: "12px 8px"  ,  justifyContent: "center"}}>
...
export default QuotationModalFromStock;
