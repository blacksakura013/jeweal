...
const ModalDayBook = ({
  ...
  // State to track which items are selected via checkbox (array for multiple selection)
  ...
  const handleOKClick = () => {
    if (selectedItemIds.length === 1) {
      ...
      // Show confirmation popup ONLY in add/edit modes when there are unsaved changes.
      // Do NOT show in pure view mode (fsmState === "saved").
      const isEditOrAddMode =
        fsmState === "initial" || fsmState === "dirty" || fsmState === "editing";

      if (isEditOrAddMode && hasUnsavedData && hasUnsavedData()) {
        setPendingEditData(editData);
        setShowConfirmDialog(true);
      } else {
        if (handleDataSelection) {
          handleDataSelection(editData);
        } else {
          handleEdit(editData);
        }
        handleClose();
  ...
  const proceedWithSelection = (confirmed) => {
    if (confirmed && pendingEditData) {
      if (handleDataSelection) {
        handleDataSelection(pendingEditData);
      } else {
  ...
  // Check if OK button should be enabled - only when exactly one item is selected
  const isOKEnabled = selectedItemIds.length === 1;
  ...
  const getSummary = (items = []) => {
    if (typeof calculateSums === "function") {
      try {
        ...
        console.error("Memo Out DayBook: calculateSums failed", error);
      }
    }
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Memo Out DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `memo_out_daybook_${timestamp}.xlsx`);
  };
            ...
            >
              ...
              <Button
                ...
                sx={{
                  ...
                  borderRadius: "4px",
                  border: "1px solid #BFBFBF",
                  bgcolor: isOKEnabled ? "#17C653" : "#E6E6E6",
                  textTransform: "none",
                  "&:hover": {
                    backgroundColor: isOKEnabled ? "#17C653" : "#E6E6E6",
                  },
                  "&.Mui-disabled": {
                    bgcolor: "#E6E6E6",
                    color: "#9A9A9A",
