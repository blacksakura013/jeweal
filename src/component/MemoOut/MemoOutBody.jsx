...
const MemoOutBody = ({
  ...
  // Fetch currencies for Summary Section
  ...
  // Simple helper to get currency code for Summary Section without breaking existing logic
  const getCurrencyForSummary = useMemo(() => {
    // If currencyCode is available, use it (when manually selected)
    if (memoInfo?.currencyCode) {
      return memoInfo.currencyCode;
    }
    // If currency ID is available, find the code from currencies list
    if (memoInfo?.currency && currencies.length > 0) {
      const currency = currencies.find(c => c._id === memoInfo.currency);
      return currency?.code || "";
    }
    return "";
  }, [memoInfo?.currencyCode, memoInfo?.currency, currencies]);

  const genInvoiceNo = async () => {
    try {
      const data = await apiRequest(
        "GET",
        "/memo-outs/next-invoice-no", {},

      );
      // setMemoInfo((prev)=>({...prev,invoice_no:data?.invoice_no}))
  ...
  const getCompanyInfo = async () => {
    try {
      ...
      setMemoInfo((prev) => {
        ...
        return { ...prev, currency: companyInfoData.currency };
      })
    } catch (error) {
          ...
          <Box sx={{ display: "flex", width: "1648px" }}>
            ...
            >
              <CalculationComponent
                ...
                calculateGrandTotal={calculateGrandTotal}
                formatNumberWithCommas={formatNumberWithCommas}
                selectedCurrency={getCurrencyForSummary}
                note={note}
                disabled={shouldDisableFields}
              />
            </Box>
