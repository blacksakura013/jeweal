...
const ModalDayBook = ({
  ...
  const isIndeterminate = false; // Never show indeterminate state
  ...
  const buildExportRows = () => {
    const rowsToExport =
      selectedItemIds.length > 0
        ? data.filter((item) => selectedItemIds.includes(item._id))
        : data;

    return rowsToExport.map((item) => {
      const summary = getSummary(item?.items || []);
      const rowIndex =
        data.findIndex((quotation) => quotation._id === item._id) + 1;
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Quotation DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `quotation_daybook_${timestamp}.xlsx`);
  };
            ...
            >
              ...
              <Button
                ...
                sx={{
                  ...
                  borderRadius: "4px",
                  border: "1px solid #BFBFBF",
                  bgcolor: isOKEnabled ? "#17C653" : "#BFBFBF",
                  textTransform: "none",
                  "&:hover": {
                    backgroundColor: isOKEnabled ? "#17C653" : "#BFBFBF",
                  },
                  "&:disabled": {
