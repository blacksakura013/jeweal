...
const CustomTextField = ({
  ...
  const [internalValue, setInternalValue] = useState(() => {
    ...
    if (!noDecimal && type === "number" && value !== "" && value !== null && value !== undefined) {
  ...
  useEffect(() => {
    // Don't format while user is typing (will format on blur instead)
    if (isTypingRef.current) {
      return;
    }

    // Format the value when it's set from props (e.g., when data comes from API)
    if (!noDecimal && type === "number" && value !== "" && value !== null && value !== undefined) {
      const num = parseFloat(value);
      if (!isNaN(num)) {
        const formatted = num.toFixed(decimal);
        // Always format to ensure correct decimal places, even if numeric value is same
        // This handles cases where value is 10 but should display as 10.00 or 10.000
        setInternalValue(formatted);
      } else {
        setInternalValue(value ?? "");
      }
    } else {
      // For non-number fields or empty values, just set the value
      const stringValue = value ?? "";
      setInternalValue(stringValue);
    }
  }, [value, decimal, noDecimal, type]);
...
const TableRowComponent = (
  ({
    ...
    const fields = [
      ...
      { key: "weight", component: CustomTextField, type: "number", decimal: 3 }, // 3 decimals per schema
      { key: "price", component: CustomTextField, type: "number", decimal: 2 }, // 2 decimals per schema
      {
        key: "unit",
        component: CustomSelectInput,
