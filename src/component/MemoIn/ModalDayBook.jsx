...
const ModalDayBook = ({
  ...
  const handleOKClick = () => {
    if (selectedItemIds.length === 1) {
      ...
      if (selectedItem) {
        ...
        if ((fsmState === "initial" || fsmState === "dirty" || fsmState === "editing") && hasUnsavedData && hasUnsavedData()) {
  ...
  const getSummary = (items = []) => {
    if (typeof calculateSums === "function") {
      try {
        ...
        console.error("Memo In DayBook: calculateSums failed", error);
      }
    }
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Memo In DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `memo_in_daybook_${timestamp}.xlsx`);
  };
