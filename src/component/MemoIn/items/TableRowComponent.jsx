...
import { grandTotalState, memoInfoState } from "recoil/MemoIn/MemoState";
...
const TableRowComponent = (
  ({
    ...
    disabled = false, // Prop to indicate if the entire document is disabled
    // rows,
    // setRows,
  }) => {
    ...
    const isDayBookEdit = memoInfo?.isDayBookEdit || false;
    ...
    const fields = [
      ...
      { key: "pcs", component: CustomTextField ,type:"number", noDecimal:true},
      ...
      >
        ...
        <Box
          ...
          onClick={() => {
            ...
            const isViewMode = isDayBookEdit && !editMemoStatus;
            if (isApprovedDoc || disabled || isViewMode) return;
            const input = document.getElementById(`memoin-pic-input-${index}`);
        ...
        >
          <Box
            sx={{
              ...
              justifyContent: "center",
              overflow: "hidden",
              cursor: (isApprovedDoc || disabled || (isDayBookEdit && !editMemoStatus)) ? "not-allowed" : "pointer",
              opacity: (isApprovedDoc || disabled || (isDayBookEdit && !editMemoStatus)) ? 0.6 : 1,
            }}
          >
            {(() => {
          ...
          <input
            id={`memoin-pic-input-${index}`}
            type="file"
            accept="image/*"
            style={{ display: "none" }}
            disabled={isApprovedDoc || disabled || (isDayBookEdit && !editMemoStatus)}
            onChange={(e) => {
        ...
        {fields.map(({ key, component: Component, ...rest }) => (
          ...
          >
            <Component
              ...
              onChange={(value) => {
                // For unit field, ensure value is lowercase to match schema enum
                let processedValue = value;
                if (key === "unit" && typeof value === "string") {
                  processedValue = value.toLowerCase();
                }
                handleChange(index, key, processedValue, rows?.[index]);
              }}
              onDropdownClick={() => {
              ...
              disabled={(() => {
                ...
                if (!isDayBookEdit && editMemoStatus) {
                  ...
                  const isFromStock = !isFromMemoPending && currentRow?.stock_id && currentRow?.isFromStock === true;
