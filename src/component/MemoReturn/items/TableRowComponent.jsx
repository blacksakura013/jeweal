...
const TableRowComponent = React.memo(
  ({
    ...
    const fdata = rows?.[index]?.isFromMemoPending ? memoInfo?.memoPendingData?.find(mp => mp._id === rows?.[index]?.uniqueId) : memoInfo?.memoReturnDayBookData?.find(mp => mp._id === rows?.[index]?.uniqueId);
    ...
    const handleChange = useCallback(
      (index, field, val, item = {}) => {
        ...
        if (field === 'pcs' && (isMemoPending || isDayBookMode)) {
      ...
      [setRows, isMemoPending, memoInfo?.memoPendingData, memoInfo?.isDayBookEdit, memoInfo?.memoReturnDayBookData]
    );
                ...
                <Component
                  ...
                  disabled={(() => {
                    ...
                    const editableFieldsInDaybook = [
                      "cer_type", "cer_no", "pcs", "weight_per_piece", "weight", "price", "unit",
                      "discount_amount", "discount_percent", "labour", "labour_price", "remark", "ref_no",
                    ];
                    
                 
                    const stoneFields = [
                      "lot_no", "stone", "shape", "size", "color", "cutting", "quality", "clarity",
                    ];
