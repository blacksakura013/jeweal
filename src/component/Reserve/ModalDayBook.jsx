...
const ModalDayBook = ({
  ...
  // Handle checkbox change and track selected items
  const handleCheckboxChangeWithTracking = (item) => {
  ...
  const handleOkClick = () => {
    if (selectedItems.length === 1) {
      ...
      if (hasUnsavedData && hasUnsavedData()) {
        setPendingSelectedItem(selectedItem);
        setShowConfirmDialog(true);
      } else {
        proceedWithSelection(selectedItem);
      }
    }
  };

 
  const proceedWithSelection = (selectedItem) => {
    if (handleDataSelection) {
      handleDataSelection(selectedItem);
    } else {
  ...
  const handleConfirmDialogClose = (confirmed) => {
    ...
    if (confirmed && pendingSelectedItem) {
      proceedWithSelection(pendingSelectedItem);
    }
    setPendingSelectedItem(null);
  };


  const isOkButtonEnabled = selectedItems.length === 1;
  ...
  const isAllSelected = hasData && selectedItemIds.length === data.length;

  const getSummary = (items = []) => {
    if (typeof calculateSums === "function") {
      try {
        const result = calculateSums(items || []);
        if (result && typeof result === "object") {
          return result;
        }
      } catch (error) {
        console.error("Reserve DayBook: calculateSums failed", error);
      }
    }
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Reserve DayBook");

    const timestamp = moment().format("YYYYMMDD_HHmmss");
    XLSX.writeFile(workbook, `reserve_daybook_${timestamp}.xlsx`);
  };
  ...
  return (
    <>
          ...
          <Box sx={style}>
                    ...
                    >
                      ...
                      >
                        <Checkbox
                          ...
                          onChange={() =>
                            handleCheckboxChangeWithTracking(item)
                          }
                        />
                        <Typography
                          sx={{
                      ...
                      >
                        <Box
                          sx={{
                            backgroundColor: item.status_approve === "approved" ? "#00AA3A33" : "#E6E6E6",
                            color: item.status_approve === "approved" ? "#00AA3A" : "#57646E",
                            padding: "6px 10px",
                            borderRadius: "5px",
                            fontFamily: "Calibri",
                            fontSize: "14px",
                        ...
                        >
                          {item.status_approve === "approved" ? "Approved" : "Unapproved"}
                        </Box>
                      </Box>
            ...
            >
              ...
              >
                <Typography
                  sx={{
                    color: isOkButtonEnabled ? "#FFF" : "#9A9A9A",
                    fontSize: "16px",
                    fontFamily: "Calibri",
                    fontStyle: "normal",
                    fontWeight: 700,
      ...
      <ConfirmCancelDialog
        ...
        message="You have unsaved changes. Selecting daybook data will replace your current data. Do you want to continue?"
        noButtonText="No"
        yesButtonText="Yes"
      />
    </>
