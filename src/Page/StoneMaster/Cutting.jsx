...
import CuttingBody from "../../component/StoneMaster/Cutting/CuttingBody";
...
import {
  cuttingFSMState,
  cuttingOriginalDataState,
  cuttingFormDataState,
} from "../../recoil/state/stone/CuttingState";
import {API_URL} from "config/config.js";

const Cutting = () => {
  const [selectedData, setSelectedData] = useState(null);
  const [responseMessage, setResponseMessage] = useState("");
  const [fsmState, setFsmState] = useRecoilState(cuttingFSMState);
  const [originalData, setOriginalData] = useRecoilState(cuttingOriginalDataState);
  const [formData, setFormData] = useRecoilState(cuttingFormDataState);
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box  sx={{marginLeft: "222px" , Height : "100vh " , paddingBottom : "130px"}}>
        ...
        <Box sx={{ display: "flex" }}>
          ...
          <SearchBarCutting onDataSelect={handleDataSelection} isEditing={isListDisabled} selectedData={formData} />
        </Box>
        ...
        <Footer
          ...
          fsmState={fsmState}
          endpointPath={method === "post" ? "/master?master_type=master_stone_cutting" : endpointPath}
          navigatePath={navigatePath}
          addNavigatePath="/stone-master/cutting/add"
          method={method}
