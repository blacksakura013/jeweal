...
import SearchBarQuality from "../../component/StoneMaster/Quality/SearchBarQuality";
import QualiyuBody from "../../component/StoneMaster/Quality/QualityBody";
import QualiyuHeader from "../../component/StoneMaster/Quality/QualityHeader";
...
import {
  qualityFSMState,
  qualityOriginalDataState,
  qualityFormDataState,
} from "../../recoil/state/stone/QualityState";
import {API_URL} from "config/config.js";

const Quality = () => {
  const [selectedData, setSelectedData] = useState(null);
  const [responseMessage, setResponseMessage] = useState("");
  const [fsmState, setFsmState] = useRecoilState(qualityFSMState);
  const [originalData, setOriginalData] = useRecoilState(qualityOriginalDataState);
  const [formData, setFormData] = useRecoilState(qualityFormDataState);
  ...
  const navigatePath = "/stone-master/quality";
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box  sx={{marginLeft: "222px" , Height : "100vh " , paddingBottom : "130px"}}>
        ...
        <Box sx={{ display: "flex" }}>
          ...
          <SearchBarQuality onDataSelect={handleDataSelection} isEditing={isListDisabled} selectedData={formData} />
        </Box>
        ...
        <Footer
          ...
          fsmState={fsmState}
          endpointPath={method === "post" ? "/master?master_type=master_stone_quality" : endpointPath}
          navigatePath={navigatePath}
          addNavigatePath="/stone-master/quality/add"
          method={method}
          ...
          payLoadData={method === "post" ? {
            ...
            master_type: "master_stone_quality",
          } : undefined}
        />
      </Box>
    </Box>
