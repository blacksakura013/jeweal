...
import {
  stoneGroupFSMState,
  stoneGroupOriginalDataState,
  stoneGroupFormDataState,
} from "../../recoil/state/stone/StoneGroupState";

const StoneGroup = () => {
  const [selectedData, setSelectedData] = useState(null);
  const [responseMessage, setResponseMessage] = useState("");
  const [fsmState, setFsmState] = useRecoilState(stoneGroupFSMState);
  const [originalData, setOriginalData] = useRecoilState(stoneGroupOriginalDataState);
  const [formData, setFormData] = useRecoilState(stoneGroupFormDataState);
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box  sx={{marginLeft: "222px" , Height : "100vh " , paddingBottom : "130px"}}>
        ...
        <Box sx={{ display: "flex" }}>
          ...
          <SearchBarStoneMaster onDataSelect={handleDataSelection} isEditing={isListDisabled} selectedData={formData}/>
        </Box>
        ...
        <Footer
          ...
          fsmState={fsmState}
          endpointPath={method === "post" ? "/master?master_type=master_stone_group" : endpointPath}
          navigatePath={navigatePath}
          addNavigatePath="/stone-master/stone-group/add"
          method={method}
