...
import ColorBody from "../../component/StoneMaster/Color/ColorBody";
...
import {
  colorFSMState,
  colorOriginalDataState,
  colorFormDataState,
} from "../../recoil/state/stone/ColorState";
import {API_URL} from "config/config.js";

const Color = () => {
  const [selectedData, setSelectedData] = useState(null);
  const [responseMessage, setResponseMessage] = useState("");
  const [fsmState, setFsmState] = useRecoilState(colorFSMState);
  const [originalData, setOriginalData] = useRecoilState(colorOriginalDataState);
  const [formData, setFormData] = useRecoilState(colorFormDataState);
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box  sx={{marginLeft: "222px" , Height : "100vh " , paddingBottom : "130px"}}> 
        ...
        <Box sx={{ display: "flex" }}>
          ...
          <SearchBarColor onDataSelect={handleDataSelection} isEditing={isListDisabled} selectedData={formData} />
        </Box>
        ...
        <Footer
          ...
          fsmState={fsmState}
          endpointPath={method === "post" ? "/master?master_type=master_stone_color" : endpointPath}
          navigatePath={navigatePath}
          addNavigatePath="/stone-master/color/add"
          method={method}
