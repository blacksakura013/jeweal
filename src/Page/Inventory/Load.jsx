...
import { memoInfoState, originalPUDataState, selectedPUItemsState } from "recoil/Load/MemoState";
import { useQuotationAccountState, QuotationInvoiceAddressState, QuotationShippingAddressState, DayBookQuotationState, QuotationtableRowsState } from "recoil/Load/LoadState";
import { loadFSMState, loadOriginalDataState, loadFormDataState } from "recoil/state/LoadFSMState";
...
const Load = () => {
  ...
  const [originalPUData, setOriginalPUData] = useRecoilState(originalPUDataState);
  const [selectedPUItems, setSelectedPUItems] = useRecoilState(selectedPUItemsState);
  ...
  React.useEffect(() => {
    ...
    if (storedSelectedPUItems) {
      try {
        ...
        console.error("Load.jsx - Error parsing selectedPUItems from localStorage:", error);
      }
    }
  }, []);
  ...
  const handleLoadSelect = (selectedLoads) => {
    ...
    if (selectedLoads && selectedLoads.length > 0) {
      ...
      if (selectedLoad.load_item && selectedLoad.load_item.length > 0) {
        const isMergeMode = selectedLoad.load_item.some(loadItem => loadItem.pu_refs && loadItem.pu_refs.length > 0);
        
        if (isMergeMode) {
          const hasOriginalData = selectedLoad.load_item.some(loadItem => loadItem.original_pcs !== undefined);
          ...
          if (hasOriginalData) {
            const puRows = selectedLoad.load_item.map((loadItem, index) => ({
              ...
              price: (selectedLoad.load_type === "merge" && loadItem.original_price === "") ? "" : (loadItem.original_price || loadItem.price || 0),
              unit: (selectedLoad.load_type === "merge" && loadItem.original_unit === "") ? "" : (loadItem.original_unit || loadItem.unit || ""),
                ...
                for (const loadItem of selectedLoad.load_item) {
                  if (loadItem.pu_refs && loadItem.pu_refs.length > 0) {
                    for (const puRef of loadItem.pu_refs) {
                      ...
                      const response = await apiRequest("GET", `/pu/${puRef.pu_id}`);
                      if (response && response.items) {
                        // Find the specific item by pu_item_id
                        const puItem = response.items.find(item => item._id === puRef.pu_item_id);
                        if (puItem) {
                          allPURows.push({
        ...
        const formRows = selectedLoad.load_item.map((item, index) => {
          ...
          const imageUrl = item.image
            ? (/^https?:\/\//.test(item.image) ? item.image : `${API_URL}${item.image}`)
            : null;
  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      ...
      setIsApproved(true);
      // Clear selected PU items when Load is approved
      setSelectedPUItems([]);
      // Clear localStorage as well
      localStorage.removeItem('selectedPUItems');
  ...
  const resetQuotationtableRowsState = useResetRecoilState(QuotationtableRowsState);
  ...
  const hasUnsavedData = useCallback(() => {
    ...
    const hasRowData = rows && rows.length > 0 && rows.some(row => 
    ...
    const hasFormData = (ref1 && ref1.trim() !== "") || 
                       ...
                       (note && note.trim() !== "") || 
                       (remark && remark.trim() !== "");
    const account = memoInfo?.account;
    ...
    if (hasRowData || hasFormData || (hasAccount && (hasRowData || hasFormData))) {
      return true;
    }
    return false;
  }, [memoInfo, rows, ref1, ref2, note, remark]);
  ...
  }, [isEditMode, fsmState, docDate, dueDate, ref1, ref2, note, remark, rows, loadItems, memoInfo, operationType, setFsmState, setOriginalData]);
  ...
  const handleCancelConfirm = useCallback((confirmed) => {
  ...
  const handleSaveSuccess = useCallback((savedData = null) => {
    console.log('[handleSaveSuccess] Called with savedData:', savedData);
    
    // Get current load items from the ref (most up-to-date source)
    const currentLoadItemsFromRef = bodyRef.current?.getCurrentLoadItems?.() || [];
    console.log('[handleSaveSuccess] Current loadItems from ref count:', currentLoadItemsFromRef.length);
    console.log('[handleSaveSuccess] Current loadItems state count:', loadItems.length);
    ...
    console.log('[handleSaveSuccess] loadData.load_item:', loadData?.load_item);
    console.log('[handleSaveSuccess] loadData.load_item length:', loadData?.load_item?.length);
    ...
    let updatedLoadItems = currentLoadItemsFromRef.length > 0 
      ? currentLoadItemsFromRef 
      : (loadItems.length > 0 ? [...loadItems] : []);
    
   
    if (loadData?.load_item && Array.isArray(loadData.load_item) && loadData.load_item.length > 0) {
      console.log('[handleSaveSuccess] Using load_item from API response, count:', loadData.load_item.length);
      updatedLoadItems = loadData.load_item.map((loadItem, index) => {
     
        const imageUrl = loadItem.image
          ? (/^https?:\/\//.test(loadItem.image) ? loadItem.image : `${API_URL}${loadItem.image}`)
          : null;
      ...
      console.log('[handleSaveSuccess] No load_item in API response, using current loadItems from ref/state');
    }
    
    console.log('[handleSaveSuccess] Final updatedLoadItems count:', updatedLoadItems.length);
    console.log('[handleSaveSuccess] updatedLoadItems:', updatedLoadItems.map(item => ({ 
      _id: item._id, 
      pu_item_id: item.pu_item_id, 
      stone: item.stone 
    })));
  ...
  }, [docDate, dueDate, ref1, ref2, note, remark, rows, loadItems, memoInfo, currentLoadId, setFsmState, setOriginalData, setFormData, setMemoInfo, setLoadItems]);
  ...
  const onHandleSave = async () => {
    ...
    try {
      const hasFiles = Array.isArray(payload?.load_item) && payload.load_item.some(r => r && r.imageFile instanceof File);
      const buildRequestBody = () => {
        ...
        const loadItemsForJson = (payload.load_item || []).map(({ imageFile, ...rest }) => rest);
        console.log(' [Load Save] JSON payload check:');
        loadItemsForJson.forEach((item, idx) => {
          const hasImageField = 'image' in item;
          const originalHasImageFile = payload.load_item[idx]?.imageFile instanceof File;
          console.log(`  Row ${idx}: has_image_field=${hasImageField}, image=${item.image}, original_has_imageFile=${originalHasImageFile}`);
        });
        ...
        (payload.load_item || []).forEach((r, idx) => {
          if (r && r.imageFile instanceof File) {
            ...
            fileCount.total++;
            fileCount.byIndex[idx] = { token, fieldKey, filename: r.imageFile.name };
        ...
        console.log(`[Load Save] Summary: Sending ${fileCount.total} image file(s) with field names:`, 
          Object.keys(fileCount.byIndex).map(idx => fileCount.byIndex[idx].fieldKey)
        );
        return formData;
      };
      ...
      if (currentLoadId) {
        ...
        savedData = await apiRequest("PUT", `/loads/${currentLoadId}`, requestBody);
      } else {
        ...
        if (rows && rows.length > 0) {
          ...
          const existingIds = new Set(
            selectedPUItems.map((item) => 
              item.pu_item_id || item._id || item.stock_id
            )
          );
        ...
        <Box sx={{ display: "flex" }}>
          <Box>
            ...
            <InventoryLoadBody
              ...
              rows={originalPUData && originalPUData.length > 0 ? originalPUData : rows}
              setRows={setRows}
              loadItems={loadItems}
