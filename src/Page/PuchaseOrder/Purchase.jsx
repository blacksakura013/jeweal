...
import { purchaseFSMState, purchaseOriginalDataState, purchaseFormDataState } from "recoil/state/PurchaseFSMState";
...
import PurchaseBody from "component/Purchase/PurchaseBody";
...
import { grandTotalState, memoInfoState, editMemoState, isApprovedState } from "recoil/Purchase/MemoState";
import { currencyState } from "recoil/state/CommonState";
import { CustomerInvoiceAddressState, SelectedInvoiceAddressState } from "recoil/Purchase/VendorState";
...
import {
  ...
  initialState,
  newAddTemplate,
} from "../../component/Purchase/Data.jsx";
...
const MemoInOrder = () => {
  ...
  const [fsmState, setFsmState] = useRecoilState(purchaseFSMState);
  const [originalData, setOriginalData] = useRecoilState(purchaseOriginalDataState);
  const [formData, setFormData] = useRecoilState(purchaseFormDataState);
  ...
  const restoreFromOriginalData = (data) => {
    ...
    if (data.currency) {
      setMemoInfo(prev => ({
        ...
        account: { label: data.account, code: data.vendor_code_id },
  ...
  const handleDiscountPercenChangeInTalble = (index, value) => {
    ...
    // dispatch({
    //   type: "UPDATE_ITEM",
    //   payload: { index, field: "discount_percent", value: newValue },
    // });
    // dispatch({
    //   type: "UPDATE_ITEM",
    //   payload: {
  ...
  // Removed local account state as account is managed in memoInfo state
  ...
  const resetInvoiceAddressState = useResetRecoilState(CustomerInvoiceAddressState);
  const resetSelectedInvoiceAddressState = useResetRecoilState(SelectedInvoiceAddressState);
  ...
  const hasUnsavedData = useMemo(() => {
    ...
    const hasShippingAddress = shippingAddress && 
                               ...
                               shippingAddress.some(addr => addr?.label && addr.label.trim() !== "");
    
    // Note: invoice_no is auto-generated, so we don't check it as "dirty"
    // Note: currency is auto-set from company profile, so we only check if it's different from default
    const hasCurrency = memoInfo?.currency && 
                       companyCurrency && 
                       memoInfo.currency !== companyCurrency;
    ...
    if (hasAccount || hasInvoiceAddress || hasShippingAddress || hasCurrency || hasFormData) {
      return true;
    }
    return false;
  }, [memoInfo, rows, ref1, ref2, note, remark, companyCurrency, selectedInvoiceAddress, shippingAddress]);

  // Use navigation guard with cleanup callback
  useTransactionNavigationGuard(hasUnsavedData, resetPurchaseState);
  ...
  const onHandleSave = async (skipReload = false) => {
    const invalidList = [];
    ...
    const body = {
      ...
      items: rows.map((el) => {
        ...
        // Only include valid Mongo ObjectId for existing items
        try {
          const isValidObjectId = typeof el._id === "string" && /^[0-9a-fA-F]{24}$/.test(el._id);
          if (isValidObjectId) {
            base._id = el._id;
          }
        } catch {}
    ...
    if (Array.isArray(rows) && rows.some(r => r && r.imageFile instanceof File)) {
      ...
      // append simple header fields (non-items/summary)
    ...
    if (postData) {
      ...
      // Only reload if not skipping (for approval process)
      // if (!skipReload) {
      //   resetData();
      // }
      
      // Return the created PU data for approval
      return postData;
    }
  };
  ...
  const handleEdit = (item) => {
    setOpen(false);
    setEditMemoStatus(false); // Keep false - Edit button will set it to true
    ...
    // Removed setAccount call as local account state is removed

    const isFromPO = item?.isFromPO || false;
    const isFromDayBook = item?.isFromMemoPending || false;
    const approvalStatus = (item?.status || "").toLowerCase();

    // Load all invoice addresses for the account if in daybook mode and not approved
    let allInvoiceAddresses = [];
    const isNotApproved = approvalStatus !== "approved";
    if (isFromDayBook && isNotApproved) {
      ...
      if (allInvoiceAddresses.length > 0) {
        // Find the saved address in the list
        ...
        // Set the selected address FIRST to prevent useEffect from overriding it
        if (savedAddress) {
          setSelectedInvoiceAddress(savedAddress);
          // Set all addresses after setting the selected one
          setInvoiceAddress(allInvoiceAddresses);
        } else {
          // If saved address not found, create it and add to list, then select it
      ...
      // Approved or not daybook - use only saved address
      setInvoiceAddress([
        {
          label: item.invoice_address,
        },
...
setTimeout(() => {
  if (
    ...
    setMemoInfo({
      ...
      // Preserve invoice_no for DayBook data, let PO data use new sequence
      invoice_no: isFromDayBook ? item?.invoice_no : memoInfo?.invoice_no,
      account: { label: item.account, code: item.vendor_code_id },
      id: item?._id,
      isDayBookEdit: isFromDayBook || false,
      isPOEdit: isFromPO || false,
      status: isFromPO ? "unapproved" : item?.status, // PO data always starts as unapproved in PU
    });
    // Only set remark and note for DayBook data, keep blank for PO data
    setRemark(isFromDayBook ? item.remark : "");
    setNote(isFromDayBook ? item.note : "");
    ...
    const formattedItems = item.items.map((el) => ({
      ...
      ref_no: item?.invoice_no, // Set ref_no to invoice_no from PO
      // Only set isFromMemoPending if the row actually came from memo pending (has stock_id)
      // If row has stock_id, it's from memo pending/stock. Otherwise, it's manually added (don't set isFromMemoPending)
      isFromMemoPending: el.stock_id ? true : (el.isFromMemoPending !== undefined ? el.isFromMemoPending : false),
      isFromPO: isFromPO, // Mark as coming from PO modal
      labour: el.labour_type || "",
      labour_unit: el.labour_unit || null,
        ...
        <Box sx={{ display: "flex" }}>
          <Box>
            ...
            <PurchaseBody
              ...
              // Removed account and setAccount props as they are unused
              onLotChange={setLot}
