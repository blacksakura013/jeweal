...
import MemoOutReturnBody from "../../component/MemoOutReturn/MemoOutReturnBody";
import MemoOutReturnHeader from "../../component/MemoOutReturn/MemoOutReturnHeader";
import {
...
import { memoOutReturnFSMState, memoOutReturnOriginalDataState, memoOutReturnFormDataState } from "recoil/state/MemoOutReturnFSMState.js";
...
const MemoOutReturn = () => {
  ...
  const accountInfoLoadable = useRecoilValueLoadable(
    getMemoOutReturnAccountInfo
  );
 ...
 const [ref1, setRef1Raw] = useState("");
  ...
  const [companyCurrency, setCompanyCurrency] = useState(null);
   ...
   const [fsmState, setFsmState] = useRecoilState(memoOutReturnFSMState);
  const [originalData, setOriginalData] = useRecoilState(memoOutReturnOriginalDataState);
  const [formData, setFormData] = useRecoilState(memoOutReturnFormDataState);
  ...
  // Reset all MemoOutReturn state
  const resetMemoOutReturnState = useCallback(() => {
  ...
  const hasUnsavedData = useMemo(() => {
    ...
    // }
    return false;
  }, [memoInfo, rows, ref1, ref2, note, remark, selectedInvoiceAddress, shippingAddress]);

  // Use navigation guard with cleanup callback
  useTransactionNavigationGuard(hasUnsavedData, resetMemoOutReturnState);
  ...
  }, [setFsmState, setOriginalData, setFormData]);
  ...
  const handleCancelEdit = () => {
    if (fsmState === "editing") {
      ...
      const hasData = rows.length > 0 || hasUnsavedData;
      if (hasData) {
        setShowCancelConfirmDialog(true);
      } else {
        setFsmState("initial");
  ...
  const handleSaveSuccess = (savedData = null) => {
    ...
    const memoOutData = savedData?.memoOut || savedData;
    const recordId = memoOutData?._id || memoOutData?.id || memoInfo?.id;
    const invoiceNo = memoOutData?.invoice_no || memoInfo?.invoice_no;
    const status = memoOutData?.status || memoInfo?.status || "unapproved";
    const statusApprove = memoOutData?.status_approve || memoInfo?.status_approve;
    ...
    if (recordId) {
      setMemoInfo(prev => ({
        ...
        status: status,
        status_approve: statusApprove || prev.status_approve,
        isDayBookEdit: prev.isDayBookEdit || false,
      }));
    }
  ...
  const fetchMemoIn = async (item) => {
    ...
    try {
      ...
      const response = await apiRequest(
        "GET",
        `/memo-outs/items?account=${item?.account || ""}`
      );
  ...
  const onHandleSave = async () => {
    ...
    let memo_out_id = null;
    if (memoInfo?.memo_out_id) {
      memo_out_id = memoInfo.memo_out_id;
    } else if (memoInfo?.isDayBookEdit && memoInfo?.memo_out_id) {
      memo_out_id = memoInfo.memo_out_id;
    } else {
      const memoOutIds = memoInfo?.memoPendingData?.map(item => item.memo_out_id).filter(id => id !== undefined) || [];
      memo_out_id = memoOutIds.length === 1 ? memoOutIds[0] : null;
    }
    ...
    let method = "POST";
    let URL = "/memo-out-returns";
    if (memoInfo?.id) {
      method = "PUT";
      URL = `/memo-out-returns/${memoInfo?.id}`;
    }
  ...
  const handleEdit = (item) => {
    ...
    if (
      accountInfoLoadable.state === "hasValue" &&
      Array.isArray(accountInfoLoadable.contents)
    ) {
      const matchedAccount = accountInfoLoadable.contents.find(
    ...
    if (allInvoiceAddresses.length > 0) {
      ...
      if (savedAddress) {
        ...
        const reordered = [savedAddress, ...remainingAddresses];
        setInvoiceAddress(reordered);
        setSelectedInvoiceAddress(savedAddress);
      } else if (savedAddressText) {
        const fallbackAddress = {
          label: item.invoice_address,
          code: "",
        };
        const combined = [fallbackAddress, ...allInvoiceAddresses];
        setInvoiceAddress(combined);
...
setTimeout(() => {
  if (
    ...
    setExchangeRate(item.exchange_rate, true);
  }
}, 900); // delay in milliseconds, adjust as needed 

        
     setRef1(item.ref_1 || "", true);
    setRef2(item.ref_2 || "", true);
    setRemark(item.remark || "", true);
    setNote(item.note || "", true);
    ...
    setMemoInfo((prev) => ({
      ...
      id: item?._id,
      memo_out_id: item?.memo_out_id,
      isDayBookEdit: true,
    }));

  
    const updatedItems = item.items.map((obj) => {
      const rawWeightPerPiece = Number(obj.weight_per_piece);
      const computedWeightPerPiece = !Number.isNaN(rawWeightPerPiece)
        ? rawWeightPerPiece
        : Number(obj.pcs)
        ? Number(obj.weight || 0) / Number(obj.pcs || 1)
        : 0;

      return {
        ...obj,
        uniqueId: obj._id,
        total_amount: Number(obj.total_amount || 0).toFixed(2),
        weight_per_piece: Number.isFinite(computedWeightPerPiece)
          ? computedWeightPerPiece.toFixed(3)
          : "0.000",
        weight: Number(obj.weight || 0).toFixed(3),
        price: Number(obj.price || 0).toFixed(2),
        discount_percent: Number(obj.discount_percent || 0).toFixed(2),
        discount_amount: Number(obj.discount_amount || 0).toFixed(2),
        labour_price: parseFloat(Number(obj.labour_price || 0).toFixed(2)),
        amount: Number(obj.amount || 0).toFixed(2),
        other_charge:
          obj.other_charge !== undefined
            ? parseFloat(obj.other_charge).toFixed(2)
            : "0.00",
      };
    });
    ...
    // Set originalData and formData after all data is loaded
    const completeData = {
      ...item,
      items: updatedItems,
