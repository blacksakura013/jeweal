...
import MemoReturnBody from "../../component/MemoReturn/MemoReturnBody";
import MemoReturnHeader from "../../component/MemoReturn/MemoReturnHeader";
import {
...
import { grandTotalState, memoInfoState } from "recoil/MemoReturn/MemoState";
...
const MemoReturn = () => {
  ...
  // Reset all MemoReturn state
  const resetMemoReturnState = useCallback(() => {
  ...
  useTransactionNavigationGuard(hasUnsavedData, resetMemoReturnState);

  // Always reset summary-related values when Memo Return loads
  useEffect(() => {
    dispatch({ type: "RESET_STATE" });
  }, []);
  ...
  const handleSaveSuccess = (savedData = null) => {
    ...
    const memoReturnData = savedData?.memoReturn || savedData;
  ...
  const handleEdit = (item) => {
    ...
    if (allInvoiceAddresses.length > 0) {
      ...
      if (savedAddress) {
        ...
        const combinedAddresses = [fallbackAddress, ...allInvoiceAddresses];
        setInvoiceAddress(combinedAddresses);
