...
import MemoInOrderBody from "component/MemoIn/MemoInOrderBody";
import MemoInOrderHeader from "../../component/MemoIn/MemoInOrderHeader";
import {
...
const MemoInOrder = () => {
  ...
  // Always reset summary-related values (discount %, discount amount, VAT, other charges)
  // when Memo In page is loaded fresh, so old values do not leak between documents.
  useEffect(() => {
  ...
  // Reset all MemoIn state
  const resetMemoInState = useCallback(() => {
  ...
  useTransactionNavigationGuard(hasUnsavedData, resetMemoInState);
  ...
  const onHandleSave = async () => {
    ...
    // Debug: Check if items have stock_id before saving
    console.log("Saving memo in items:", body.items);
  ...
  const handleSaveSuccess = (savedData = null) => {
        ...
        return prevRows.map((prevRow, index) => {
          ...
          if (savedItem && savedItem.stock_id) {
            ...
            return {
              ...
              _id: savedItem._id || savedItem.id || prevRow._id, 
            };
          }
          return prevRow;
        });
    ...
    if (recordId) {
      setMemoInfo(prev => {
        ...
        console.log("Setting memoInfo:", updated);
        return updated;
      });
    }
  ...
  const handleEdit = (item) => {
    ...
    // Set shipping address (fallback to saved one)
    setShippingAddress([
      {
...
setTimeout(() => {
  if (
    ...
    // Debug: Check if items have stock_id
      ...
      <ConfirmCancelDialog
        ...
        onClose={(confirmed) => {
          ...
          if (fsmState === "editing" && originalData) {
            setDocDate(originalData.doc_date);
            setDueDate(originalData.due_date);
