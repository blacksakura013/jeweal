...
import MemoOutBody from "../../component/MemoOut/MemoOutBody";
import MemoOutHeader from "../../component/MemoOut/MemoOutHeader";
import {
...
import { grandTotalState, memoInfoState } from "recoil/MemoOut";
import { currencyState } from "recoil/state/MemoOutCommonState";
import {
  initialData,
  initialData2,
  initialState,
...
import { memoOutFSMState, memoOutOriginalDataState, memoOutFormDataState } from "recoil/state/MemoOutFSMState.js";
import { getMemoOutAccountInfo } from "recoil/selector/MemoOutAccountSelector";
import { editMemoState } from "recoil/MemoOut.js";
...
const MemoOut = () => {
  ...
  const accountInfoLoadable = useRecoilValueLoadable(getMemoOutAccountInfo);
  ...
  const [fsmState, setFsmState] = useRecoilState(memoOutFSMState);
  const [originalData, setOriginalData] = useRecoilState(memoOutOriginalDataState);
  const [formData, setFormData] = useRecoilState(memoOutFormDataState);
  ...
  // Reset all MemoOut state
  const resetMemoOutState = useCallback(() => {
  ...
  useTransactionNavigationGuard(hasUnsavedData, resetMemoOutState);
  ...
  const handleSaveSuccess = (savedData = null) => {
    ...
    if (recordId || rows.length > 0) {
      const completeData = {
        ...
        shipping_address: selectedShippingAddress?.label || shippingAddress?.[0]?.label || "",
        currency: {
      ...
      items: rows.map((el) => {
        ...
        try {
          ...
          if (isValidObjectId) {
            ...
            // Fallback to el.id if _id is not valid ObjectId
            const isValidId = typeof el.id === "string" && /^[0-9a-fA-F]{24}$/.test(el.id);
            if (isValidId) {
              base._id = el.id;
            }
          }
  ...
  const handleEdit = (item) => {
    ...
    // Set FSM state to saved and populate originalData/formData
    setFsmState("saved");
    setEditMemoStatus(false);
    ...
    const updatedItems = item.items.map((obj) => {
      ...
      return {
        ...
        labour_price: parseFloat(Number(obj.labour_price).toFixed(2)),
