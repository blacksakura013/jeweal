...
import SaleBody from "../../component/Sale/SaleBody";
import SalerHeader from "../../component/Sale/SaleHeader";
import {
  useQuotationAccountState,
  QuotationInvoiceAddressState,
  QuotationShippingAddressState,
} from "recoil/Sale/SaleState";
import { grandTotalState, memoInfoState , keyEditState, editMemoState } from "recoil/Sale/MemoState";
import {
  saleFSMState,
  saleOriginalDataState,
  saleFormDataState,
...
import { QuotationtableRowsState  ,DayBookQuotationState} from "recoil/Sale/SaleState";
...
const SalePage=()=>{
    ...
    const [fsmState, setFsmState] = useRecoilState(saleFSMState);
    const [originalData, setOriginalData] = useRecoilState(saleOriginalDataState);
    const [formData, setFormData] = useRecoilState(saleFormDataState);
  ...
  const handleDelete = (id) => {
    ...
    setRows((prevRows) => {
      const newRows = prevRows.filter((row) => row._id !== id);
      
     
      if (newRows.length === 0 && memoInfo?.from_reserve) {
        setMemoInfo(prev => ({
          ...prev,
          from_reserve: false,
          reserve_id: null
  ...
  const handleAddRow = () => {
      ...
      setRows((prevRows) => {
        const newItemIndex = prevRows.length + 1; 
        let newName = `New Item ${newItemIndex}`; 
        let _id = "id" + newItemIndex;
        
       
        if (memoInfo?.from_reserve) {
          setMemoInfo(prev => ({
            ...prev,
            from_reserve: false,
            reserve_id: null
  ...
  const calculateGrandTotal = () => {
    ...
    // Reset all Sale state
      const resetSaleState = useCallback(() => {
        ...
        }, [resetSaleState, setFsmState, setFormData, setOriginalData]);
      ...
      useTransactionNavigationGuard(hasUnsavedData, resetSaleState);
    
        const fetchMemoIn = async (item) => {
          console.log(item , "itesmmmm")
          try {
            setLoading(true); // Set loading to true before fetch
            const response = await apiRequest(
              "GET",
              `/reserves/by-account?account=${item?.account || ""}`
            );
            console.log(response , "respobnse")
            // setRowData(response);
            setMemoInfo((prev) => ({
              ...prev,
      ...
      const handlePost = () => {
       ...
       useEffect(() => {
        ...
        const onHandleSave = async () => {
          ...
          if (memoInfo?.currency !== companyCurrency && (!isNumeric(exchangeRate) || parseFloat(exchangeRate) <= 0)) {
            invalidList.push({
              row: "Header",
              list: ["Exchange Rate"],
            });
          ...
          const body = {
            ...
            invoice_address: invoiceAddress?.[0]?.label,
            ...
            remark: remark,
            note: note,
            // Add from_reserve flag only when there are actual reserve items
            ...(memoInfo?.from_reserve && {
              from_reserve: true
            }),
            items: rows.map((el) => {
              ...
              return {
                ...
                ...(memoInfo?.from_reserve && el?.isFromStock === true && { 
                  from_reserve: true,
                  reserve_id: memoInfo?.reserve_id 
                }),
       ...
       const handleEditToggle = () => {
          if (fsmState === "saved" && memoInfo?.id) {
          
            const currentData = {
              _id: memoInfo.id,
              invoice_no: memoInfo.invoice_no,
              account: account || memoInfo.account, // Use account state if exists, otherwise memoInfo.account
              vendor_code_id: typeof account === 'object' ? account.code : (memoInfo.account?.code || ""),
              invoice_address: invoiceAddress?.[0]?.label || "",
              shipping_address: shippingAddress?.[0]?.label || "",
              currency: typeof memoInfo.currency === 'object' ? memoInfo.currency : { _id: memoInfo.currency, code: memoInfo.currencyCode },
              doc_date: docDate,
              due_date: dueDate,
        ...
        const handleCancelEdit = () => {
          ...
          if (fsmState === "editing") {
            ...
            if (originalData) {
              ...
              setDocDate(originalData.doc_date || new Date());
              setDueDate(originalData.due_date || new Date());
              ...
              const accountValue = originalData.account && typeof originalData.account === 'object'
                ? originalData.account
                : { label: originalData.account || "", code: originalData.vendor_code_id || "" };
              ...
              const invoiceAddrValue = originalData.invoice_address || "";
              setInvoiceAddress([{ label: invoiceAddrValue }]);
              
             
              const shippingAddrValue = originalData.shipping_address || "";
              setShippingAddress([{ label: shippingAddrValue }]);
              
             
              const currencyValue = typeof originalData.currency === 'object' 
                ? originalData.currency 
                : { _id: originalData.currency || memoInfo.currency, code: originalData.currencyCode || memoInfo.currencyCode };
              setMemoInfo(prev => ({
                ...prev,
                currency: currencyValue._id,
                currencyCode: currencyValue.code,
              }));
        ...
        const handleSaveSuccess = () => {
          ...
          if (memoInfo?.id) {
            ...
            const originalDataForEdit = {
              ...memoInfo,
              _id: memoInfo.id || memoInfo._id,
              account: typeof memoInfo.account === 'object' ? memoInfo.account.label : memoInfo.account,
              vendor_code_id: typeof memoInfo.account === 'object' ? memoInfo.account.code : memoInfo.vendor_code_id,
              invoice_address: invoiceAddress?.[0]?.label || "",
              shipping_address: shippingAddress?.[0]?.label || "",
              currency: typeof memoInfo.currency === 'object' ? memoInfo.currency : { _id: memoInfo.currency, code: memoInfo.currencyCode },
              doc_date: docDate,
            ...
            setOriginalData(originalDataForEdit);
            setFormData(originalDataForEdit);
          }
        };
        ...
        if (
          ...
          setMemoInfo({
            ...
            isDayBookEdit: true,
            status: item?.status,
            status_cancel: item?.status_cancel
          });
        ...
        const handleReserveSubmit = (selectedStockRows = []) => {
          ...
          const remainingRows = [...rows, ...newRows];
          const hasReserveItems = remainingRows.some(item => item?.isFromStock === true);
          ...
          if (!hasReserveItems) {
            ...
            if (memoInfo?.ref1 && memoInfo.ref1.startsWith('OD')) {
              setRef1('');
            }
          }
        };
        ...
        const handleReturnReserveEdit = (reserveItem) => {
          setDocDate(new Date(reserveItem.doc_date));
          setDueDate(new Date(reserveItem.due_date));
          setInvoiceAddress([
            {
              label: reserveItem.invoice_address || "",
            },
          ]);
          setShippingAddress([
            {
              label: reserveItem.shipping_address || "",
            },
          ]);
      
          setTimeout(() => {
            if (
              reserveItem.exchange_rate === 0 ||
              reserveItem.exchange_rate === "0" ||
              reserveItem.exchange_rate === "" ||
              reserveItem.exchange_rate === null
            ) {
              setExchangeRate("");
            } else {
              setExchangeRate(reserveItem.exchange_rate);
            }
          }, 900);
    
          setRef1(reserveItem.invoice_no || ""); 
          setRef2(reserveItem.ref_2 || "");
      
          setMemoInfo({
            ...memoInfo,
            currency: reserveItem?.currency?._id,
            currencyCode: reserveItem?.currency?.code,
            exchange_rate: parseFloat(reserveItem?.exchange_rate || 1).toFixed(2),
            account: { label: reserveItem.account, code: reserveItem.vendor_code_id },
            invoice_no: "", 
            id: reserveItem?._id,
            isDayBookEdit: false, 
            reserve_id: reserveItem?._id, 
            from_reserve: true 
          });
          ...
          (async () => {
            try {
              ...
              console.error("Error in immediate invoice generation:", error);
            }
          })();
          ...
          setRemark(reserveItem.remark || "");
          setNote(reserveItem.note || "");
      
         
          const formattedItems = reserveItem.items.map((item) => {
            ...
            if (item.image) {
              ...
              } else if (typeof item.image === 'string' && /^https?:\/\//.test(item.image)) {
                imageUrl = item.image;
                imagePath = item.image.replace(API_URL, '');
              } else {
                imagePath = item.image;
                imageUrl = item.image;
              }
            ...
            return {
              ...
              labour: item.labour || "Recutting",
              labour_price: parseFloat(Number(item.labour_price || 0).toFixed(2)),
              other_charge: item.other_charge !== undefined ? parseFloat(Number(item.other_charge || 0).toFixed(2)) : "0.00",
              remark: item.remark || "",
              account: reserveItem.account,
              location: "",
              isFromStock: true, 
              image: imagePath,
              image_preview: imageUrl,
          ...
          handleCurrencyChange(reserveItem?.currency?.code);
      
          const { summary = {} } = reserveItem;
          ...
          dispatch({
            type: "SET_DISCOUNT_AMOUNT",
            payload: summary.discount_amount || 0,
          });
          ...
          // Handle VAT
          if (summary.vat_amount > 0 || summary.vat > 0) {
            dispatch({ type: "SET_VAT_AMOUNT", payload: summary.vat || 0 });
            dispatch({ type: "TOGGLE_VAT", payload: true });
          }
        }
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box sx={{ marginLeft: "222px", Height: "100vh ", paddingBottom: "130px" }}>
        ...
        {/* <Footer handlePost={handlePost} onCancelEdit={resetData} /> */}
              ...
              if (pendingSelection === "CANCEL_EDIT") {
                if (fsmState === "editing") {
                  if (originalData) {
                    ...
                    const accountValue = typeof originalData.account === 'object' 
                      ? originalData.account 
                      : { label: originalData.account || "", code: originalData.vendor_code_id || "" };
                    setAccount(accountValue);
                    setMemoInfo(prev => ({
