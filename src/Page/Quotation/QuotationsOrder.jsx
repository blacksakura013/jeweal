...
import QuotationsOrderBody from "component/Quotations/QuotationsOrderBody";
import QuotationsOrderHeader from "../../component/Quotations/QuotationsOrderHeader";
import {
...
import { grandTotalState, memoInfoState, editMemoState } from "recoil/MemoState";
import { currencyState } from "recoil/state/CommonState";
import {
  quotationFSMState,
  quotationOriginalDataState,
  quotationFormDataState,
} from "../../recoil/state/QuotationFSMState";
import {
  initialData,
  initialData2,
  initialState,
...
const QuotationsOrder = () => {
  ...
  // FSM States
  const [fsmState, setFsmStateOriginal] = useRecoilState(quotationFSMState);

  const setFsmState = setFsmStateOriginal;
  const [originalData, setOriginalData] = useRecoilState(quotationOriginalDataState);
  const [formData, setFormData] = useRecoilState(quotationFormDataState);
  ...
  // Clear address states when component unmounts
  useEffect(() => {
    return () => {
      setInvoiceAddress([{ code: "", label: "" }]);
      setShippingAddress([{ code: "", label: "" }]);
  ...
  const handleNumberChange = (index, field, value) => {
    ...
    // Ensure labour_price is always stored as number with 2 decimals
    if (field === "labour_price") {
      const fixedValue = parseFloat(newValue.toFixed(2));
      handleUpdate(index, field, fixedValue);
    } else {
  ...
  const setExchangeRate = (value, isSystemSet = false) => {
    if (value === 0 || value === "0" || value === "" || value === null) {
      setExchangeRateRaw("");
    } else {
      setExchangeRateRaw(value);
    }
    // Only trigger FSM state change when user manually edits, not during system initialization
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  };
  const [ref1, setRef1Raw] = useState("");
  const [ref2, setRef2Raw] = useState("");
  const [account, setAccountRaw] = useState("");
  const [lot, setLotRaw] = useState("");
  const [stone, setStoneRaw] = useState("");
  const [note, setNoteRaw] = useState("");
  const [remark, setRemarkRaw] = useState("");

  // Wrapper setters to trigger FSM state changes
  const setRef1 = (value, isSystemSet = false) => {
    setRef1Raw(value);
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  ...
  const setRef2 = (value, isSystemSet = false) => {
    setRef2Raw(value);
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  ...
  const setAccount = (value, isSystemSet = false) => {
    setAccountRaw(value);
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  ...
  const setLot = (value, isSystemSet = false) => {
    setLotRaw(value);
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  ...
  const setStone = (value, isSystemSet = false) => {
    setStoneRaw(value);
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  ...
  const setNote = (value, isSystemSet = false) => {
    setNoteRaw(value);
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  ...
  const setRemark = (value, isSystemSet = false) => {
    setRemarkRaw(value);
    if (!isSystemSet && fsmState === "initial") {
      setFsmState("dirty");
    }
  ...
  useEffect(() => {
    if (memoInfo?.id || memoInfo?.isDayBookEdit) {
      setFsmState("saved");
      setFormData(memoInfo);
      setOriginalData(memoInfo);
      setSelectedData(memoInfo);
    } else {
      setFsmState("initial");
      setFormData(null);
  ...
  }, [memoInfo?.id, memoInfo?.isDayBookEdit]);

  const hasUnsavedQuotationData = () => {

    return (

      (rows && rows.length > 0 && rows.some(row =>
  ...
  // Direct data selection from daybook (bypasses main page confirmation)
  ...
  const handleConfirmDialogClose = (confirmed) => {
    ...
    if (confirmed) {
      if (pendingSelection === "OPEN_DAYBOOK") {
        // User confirmed to proceed with opening daybook, clear data and open modal
        setSelectedData(null);
        setOriginalData(null);
        setFormData(null);
        setFsmState("initial");
        resetQuotationState();
        setOpen(true);
      } else if (pendingSelection === "CANCEL_EDIT") {
        // User confirmed to cancel and lose unsaved data
        proceedWithCancel();
      } else {
        // Normal data selection flow
  ...
  const handleAddClick = () => {
    ...
    setFormData(null);
    setFsmState("initial");
    // Reset reducer state (summary values)
    dispatch({ type: "RESET_STATE" });
    resetQuotationState();
  };

  const handleCancelEdit = () => {
    // Check if there's unsaved stone/stock data before canceling
    if (hasUnsavedQuotationData()) {
    ...
    // No unsaved data, proceed with cancel directly
    proceedWithCancel();
  };

  const proceedWithCancel = () => {
    if (fsmState === "editing") {
      // When canceling from edit mode, go back to view mode (saved state)
      // Restore original data and go back to "saved" state
      setFormData(originalData);
      setSelectedData(originalData);
      setFsmState("saved");
      setEditMemoStatus(false); // Reset editMemoStatus
    } else if (fsmState === "dirty") {
      // When canceling from dirty state, clear everything and go to initial
      setSelectedData(null);
      setOriginalData(null);
      setFormData(null);
      setFsmState("initial");
  ...
  const handleDayBookOpen = () => {
    // Always open daybook directly - no popup here
    setOpen(true);
  };
  ...
  const hasUnsavedData = useMemo(() => {
    ...
    // Check if there are rows with actual data (not just empty rows)
    ...
    // Check if account has been selected (has code or label with actual values)
    const hasAccount = memoInfo?.account &&
      ((memoInfo.account.code && memoInfo.account.code.trim() !== "") ||
        (memoInfo.account.label && memoInfo.account.label.trim() !== ""));

    // Check if invoice address has been selected (not null and has label)
    const hasInvoiceAddress = selectedInvoiceAddress &&
      selectedInvoiceAddress.label &&
      selectedInvoiceAddress.label.trim() !== "";

    // Check if shipping address has been selected (not null and has label)
    const hasShippingAddress = selectedShippingAddress &&
      selectedShippingAddress.label &&
      selectedShippingAddress.label.trim() !== "";

    // Note: invoice_no is auto-generated, so we don't check it as "dirty"
    // Note: currency is auto-set from company profile, so we don't check it as "dirty" at all
    // const hasCurrency = false; // Currency is always default, never considered as unsaved data

    // Check if form fields have actual values
    const hasFormData = (ref1 && ref1.trim() !== "") ||
      (ref2 && ref2.trim() !== "") ||
      (note && note.trim() !== "") ||
      (remark && remark.trim() !== "");

    if (hasAccount || hasInvoiceAddress || hasShippingAddress || hasFormData) {
      return true;
    }
    return false;
  }, [memoInfo, rows, ref1, ref2, note, remark, companyCurrency, selectedInvoiceAddress, selectedShippingAddress]);

  // Reset FSM state to initial after component mounts and data loads
  useEffect(() => {
    // Add a small delay to ensure all initialization is complete
    const timer = setTimeout(() => {
      // Only reset to initial if we're in dirty state and there's no actual user data
      if (fsmState === "dirty" && !hasUnsavedQuotationData()) {
        setFsmState("initial");
      }
  ...
  // Use navigation guard with cleanup callback
  useTransactionNavigationGuard(hasUnsavedQuotationData, resetQuotationState);
  ...
  const handleChange = (index, field, value) => {
    ...
    // Trigger FSM state change when user starts editing
    if (fsmState === "initial") {
      setFsmState("dirty");
    }
  };

  // Keeps compatibility with body effect that calls onCurrencyChange
  ...
  const onHandleSave = async () => {
    ...
    const body = {
      invoice_no:
        memoInfo?.invoice_no && memoInfo?.isDayBookEdit
          ? memoInfo?.invoice_no
          : "",
      ...
      note: note,
      items: rows.map((el) => {
        // Determine valid _id for backend image matching
        let itemId = null;
    ...
    if (memoInfo?.id || memoInfo?._id) {
      method = "PUT";
      const quotationId = memoInfo?.id || memoInfo?._id;
    ...
    try {
      ...
      if (postData && !postData.error) {
        ...
        setMemoInfo(prevMemoInfo => ({
          ...prevMemoInfo,
          id: postData._id || postData.id || prevMemoInfo?.id,
          _id: postData._id || postData.id || prevMemoInfo?._id,
          invoice_no: postData.invoice_no || prevMemoInfo?.invoice_no,
        }));
  ...
  const handleEditQuotation = (item) => {
    ...
    setTimeout(() => {
      if (
        ...
        setExchangeRate("", true); // isSystemSet = true
      } else {
        setExchangeRate(item.exchange_rate, true); // isSystemSet = true
      }
    }, 800); // delay in milliseconds, adjust as needed

    setRef1Raw(item.ref_1);
    setRef2Raw(item.ref_2);
    setMemoInfo({
      ...memoInfo,
    ...
    setRemarkRaw(item.remark);
    setNoteRaw(item.note);

    // Format numeric fields to two decimals before setting rows
    const formattedItems = item.items.map((el) => {
      // Handle image: prefix relative paths with API_URL for display
      let imageUrl = null;
      let imagePath = null;
      if (el.image) {
        if (typeof el.image === 'string' && el.image.startsWith('/uploads/')) {
          imagePath = el.image;
          imageUrl = `${API_URL}${el.image}`;
        } else if (typeof el.image === 'string' && /^https?:\/\//.test(el.image)) {
          imageUrl = el.image;
          imagePath = el.image.replace(API_URL, '');
        } else {
          imagePath = el.image;
          imageUrl = el.image;
        }
    ...
    <Box sx={{ display: "flex" }}>
      ...
      >
        ...
        <Box sx={{ display: "flex" }}>
          <Box>
            <QuotationsOrderHeader
              ...
              hasUnsavedQuotationData={hasUnsavedQuotationData}
              open={open}
              setOpen={setOpen}
            />
        ...
        <FooterVendor
          ...
          isDayBookEdit={memoInfo?.isDayBookEdit}
        />
...
export default QuotationsOrder;
