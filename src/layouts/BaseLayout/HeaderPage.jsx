...
import { editStatusState,isEditingState } from "recoil/state/CommonState";
import { useRecoilState } from "recoil";
import { useParams } from "react-router-dom";
const HeaderPage = (props) => {

    //{ selectedData, isEditing, onEditToggle }
    const [isEditing, setIsEditing] = useRecoilState(isEditingState);

  const [editStatus,setEditStatus] = useRecoilState(editStatusState);
  const fillColor = editStatus ? "#57646E" : "#FFF";
  ...
  return (
    <>
      ...
      >
        ...
        <Box sx={{ float:"right", display: "flex", alignItems: "center", gap: "12px" }}>
          <Box>



            <Link to={isEditing ? "#" : (props.baseUrl?props.baseUrl+"/add":"")} onClick={(e) => isEditing && e.preventDefault()} reloadDocument={!isEditing}>
              <Button
                disabled={isEditing}
                sx={{
                  textTransform: "none",
                  ...
                  gap: "8px",
                  marginRight: "12px",
                  backgroundColor: isEditing ? "#E6E6E6" : "#C6A969",
                  "&:hover": {
                    backgroundColor: isEditing ? "#E6E6E6" : "#C6A969",
                  },
                }}
              >
                <Typography
                  sx={{
                    color: isEditing ? "#57646E" : "var(--jw-background-white-textwhite, #FFF)",
                    fontFamily: "Calibri",
                    fontSize: "16px",
                    fontStyle: "normal",
                    fontWeight: 700,
            ...
            <Link to={props.baseUrl?props.baseUrl+"/list":""} reloadDocument >
            <Button
              sx={{
                textTransform: "none",
                height: "35px",
...
{props.hasEditButton?          <Box>
            <Button
              ...
              sx={{
                ...
                gap: "8px",
                marginRight: "12px",
                backgroundColor: action!=="edit" ? "#E6E6E6" : "#C6A969",
                "&:hover": {
                  backgroundColor: action!=="edit" ? "#E6E6E6" : "#C6A969",
                },
              }}
              disabled={action!=="edit"}
              className={action!=="edit"?"edit_data_button active":"edit_data_button disabled"}
            >
              <svg
...
{props.modalList?   
  <Box>
            <ModalListItem 
               ...
               modalListHeader={props.modalListHeader}
               modalListSubHeader={props.modalListSubHeader}
               modalListEndpoint={props.modalListEndpoint}
               modalUpdateStatusEndpoint={props.modalUpdateStatusEndpoint}
               modalStatusCustomField={props.modalStatusCustomField}
               
               // modalListExportEndpoint=""
               // modalListSearchEndpoint=""
               modalListSearchCustomFieldHeader={props.modalListSearchCustomFieldHeader}
               modalListSearchCustomFieldName={props.modalListSearchCustomFieldName}
