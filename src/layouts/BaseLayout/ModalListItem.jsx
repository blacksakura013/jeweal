...
import SwitchIOSStyleInModal from "../../component/SwitchIOSStyleInModal";
...
const ModalListItem = (props) => {
  ...
  const {modalStatusCustomField} = props
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(props.modalListEndpoint){
             const response = await apiRequest(
          "GET",
          props.modalListEndpoint
        );
        setData(response);
        }
  ...
  const handleSwitchChange =async  (id,status)  => {
    ...
    try {
      ...
      const url = id
      ? (props.modalUpdateStatusEndpoint || "").replace("/id/", `/${id}/`)
      : props.modalUpdateStatusEndpoint;
      ...
      setData((prevData) =>
        prevData.map((item) =>
          item._id === id
            ? { ...item, status: item.master_status }
            : item
        )
      );
    }
          ...
          <Box sx={style}>
            ...
            >
              ...
              >
               ...
               <TableContainer >
      <Table sx={{ minWidth: 650 }} aria-label="simple table" className="table_list_data modal_table">
        <TableHead>
          <TableRow>
            <TableCell>   
                 <Checkbox checked={selectAll}
    onChange={handleSelectAllChange}/> #</TableCell>
    {props.modalListSearchCustomFieldHeader.map((v,k)=>{
      return (
                    <TableCell key={k}><b>{v}</b></TableCell>
...
<TableRow
  ...
  /></TableCell>
  {props.modalListSearchCustomFieldName.map((v,k)=>{
    return (
    <TableCell key={k}>
{arr[key][v]}
